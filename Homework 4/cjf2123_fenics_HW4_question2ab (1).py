...
for i in range(len(N)):
    ...
    u0 = Expression("exp(x[0]+x[1]/2.0)") #Constant(0.0)
    bc = DirichletBC(V, u0, boundary)
    
    # Define variational problem
    u_e = Expression("exp(x[0]+x[1]/2.0)")
    u = TrialFunction(V)
    v = TestFunction(V) # F = int(v(Lu-f)dx). Try to make this zero. Least square problem
    f = Expression("1.25*exp(x[0]+x[1]/2.0)") #what needs #to be appended. The is the forcing term .
...
plot(u32, interactive=True, axes=True, 
         title='surf plot of u, N = 32',colorbar='on',)
