...
x = np.arange(a,bx,hx)   # grid points x including boundaries
y = np.arange(a,by,hy)    # grid points y including boundaries
if len(y) < my+2:
    y1 = np.zeros((my+2))
    y1[0:len(y)] = y
    y1[-1] = by
...
utrue = np.exp(X+Y/2)
usoln = utrue
rhs[:,0] -= usoln[1:-1,0] / (hx*hy)
rhs[:,-1] -= usoln[1:-1,-1] / (hx*hy)
rhs[0,:] -= usoln[0,1:-1] / (hx*hy)
rhs[-1,:] -= usoln[-1,1:-1] / (hx*hy)  
...
A = (sp.kron(I,T) + sp.kron(S,I2)) / (hx*hy)
A = A.tocsr()
...
#        umax = usoln.max()
umax = uvec.max()
err2norm = np.sqrt(hx*hy)*np.linalg.norm(umax-umax_true)
...
print "mx = {0},my = {1}, hx = {2}, hy = {3} ".format(mx,my,hx,hy)
